...
//  D5
//
//  Created by Joseph DeWeese on 8/15/24.
//
...
HStack(spacing: -25) {
    ...
    VStack{
        ...
        .offset(x: animateCurrentWorkout && selectedWorkout?.id == self.id ? -20 : 0)
        
        /// Book Cover Image
        ZStack {
            if !(showDetailView && selectedWorkout?.id == self.id) {
